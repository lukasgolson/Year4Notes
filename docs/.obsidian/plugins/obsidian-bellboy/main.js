...
// src/organize-file-structure.ts
var import_obsidian = __toModule(require("obsidian"));
var FileStatus;
(function(FileStatus2) {
  FileStatus2["Inbox"] = "inbox";
  FileStatus2["InProgress"] = "in-progress";
  FileStatus2["Planning"] = "planning";
  FileStatus2["Blocked"] = "blocked";
  FileStatus2["Backlog"] = "backlog";
  FileStatus2["Done"] = "done";
})(FileStatus || (FileStatus = {}));
var FileMode;
(function(FileMode2) {
  FileMode2["Inbox"] = "01_inbox";
  FileMode2["Working"] = "02_working";
  FileMode2["Archive"] = "03_archive";
})(FileMode || (FileMode = {}));
var mapFileStatusToMode = (status) => {
  switch (status) {
...
var createFolderIfNotExists = (_0) => __async(void 0, [_0], function* ({ plugin, path }) {
  const abstractFile = plugin.app.vault.getAbstractFileByPath(path);
  if (!abstractFile)
    yield plugin.app.vault.createFolder(path);
});
var organizeFileStructure = (_0) => __async(void 0, [_0], function* ({
  plugin,
  file,
  cache
}) {
  const { status = FileStatus.Inbox } = cache == null ? void 0 : cache.frontmatter;
  ...
  if (!isSupportedStatus || isExcluded)
    return;
  ...
  yield createFolderIfNotExists({ plugin, path: folder });
  const fileName = `${folder}/${file.name}`;
  if (file.path !== fileName && plugin.settings.shouldOrganizeFileStructure) {
    yield plugin.app.fileManager.renameFile(file, fileName);
  }
});
var stashAttachments = (_0) => __async(void 0, [_0], function* ({
  plugin,
  file
}) {
...
var refreshFileStructure = (_0) => __async(void 0, [_0], function* ({ plugin }) {
  const files = plugin.app.vault.getFiles();
  yield Promise.allSettled(files.map((file) => __async(void 0, null, function* () {
    const cache = plugin.app.metadataCache.getFileCache(file);
    yield organizeFileStructure({ plugin, file, cache });
    yield stashAttachments({ plugin, file });
  })));
});
...
var loadSettings = (plugin) => __async(void 0, null, function* () {
  const loadedPluginData = yield plugin.loadData();
  plugin.settings = __spreadValues(__spreadValues({}, DEFAULT_SETTINGS), loadedPluginData);
});
var saveSettings = (plugin) => __async(void 0, null, function* () {
...
var SettingsTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.createEl("p").setText("Bellboy is built to be an opinionated file structure manager for Obsidian,\nit follows an automated file system structure that implemets the Idea Processor method");
    const link = containerEl.createEl("a");
    link.setText("Two Minute Intro \u{1F6CE}");
    link.setAttribute("href", "https://github.com/shakedlokits/obsidian-bellboy/blob/master/README.md");
    containerEl.createEl("h1").setText("\n");
    new import_obsidian2.Setting(containerEl).setName("Should Update File Names").setDesc("Bellboy uses an simplified naming convention based on the file's header and icon.").addToggle((toggle) => toggle.setValue(this.plugin.settings.shouldUpdateFileNames).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.shouldUpdateFileNames = value;
      yield saveSettings(this.plugin);
    })));
    new import_obsidian2.Setting(containerEl).setName("Should Organize Files").setDesc("Bellboy organizes files based on the Idea Processor state system.").addToggle((toggle) => toggle.setValue(this.plugin.settings.shouldOrganizeFileStructure).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.shouldOrganizeFileStructure = value;
      yield saveSettings(this.plugin);
    })));
    new import_obsidian2.Setting(containerEl).setName("Refresh File Structure").setDesc("Run a full file structure organization.").setDisabled(!this.plugin.settings.shouldOrganizeFileStructure).addButton((button) => button.setButtonText("Run Refresh").onClick(() => __async(this, null, function* () {
      return refreshFileStructure({ plugin: this.plugin });
    })));
  }
};
...
// src/update-file-name.ts
var import_lodash = __toModule(require_lodash());
var updateFileName = (_0) => __async(void 0, [_0], function* ({
  plugin,
  file,
  cache
}) {
  ...
  if (isExcluded)
    return;
  const [{ heading: title }] = (_a = cache == null ? void 0 : cache.headings) != null ? _a : [{ heading: file.basename }];
  const { icon } = cache == null ? void 0 : cache.frontmatter;
  const fileName = `${(0, import_lodash.kebabCase)(title)}${icon ? "-" + (0, import_lodash.trim)(icon) : ""}.md`;
...
// main.ts
var Bellboy = class extends import_obsidian3.Plugin {
  onload() {
    return __async(this, null, function* () {
      yield loadSettings(this);
      this.addSettingTab(new SettingsTab(this.app, this));
      this.registerEvent(this.app.metadataCache.on("changed", (file, data, cache) => __async(this, null, function* () {
        yield updateFileName({ plugin: this, file, cache });
        yield organizeFileStructure({ plugin: this, file, cache });
      })));
      this.registerEvent(this.app.vault.on("create", (file) => __async(this, null, function* () {
        yield stashAttachments({ plugin: this, file });
      })));
    });
  }
};
